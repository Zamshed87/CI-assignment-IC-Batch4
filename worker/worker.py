...
import boto3
import redis
from config import DATABASE_URL, QUEUE_URL, REDIS_HOST, REDIS_PORT, SQS_REGION
...
def update_all_todos_cache(session):
    ...
    try:
        ...
        cache_data = {"todos": todos_list, "_cached_at": datetime.utcnow().isoformat()}
...
def process_notification(message):
    try:
        ...
        logger.info(
            "Notification details - ID: %s, Action: %s, Data: %s",
            todo_id,
            action,
            todo_data,
        )
        ...
        try:
            if action == "todo_created":
                ...
                if existing_todo:
                    logger.info(
                        "Todo with ID %s already exists, skipping creation", todo_id
                    )
                ...
                if not todo_data.get("title"):
                    logger.error(
                        "Title is required for todo creation. Available fields: %s",
                        list(todo_data.keys()),
                    )
                    return False
        ...
        logger.error(
            "Error processing notification: %s - Error: %s",
            message,
            str(e),
            exc_info=True,
        )
