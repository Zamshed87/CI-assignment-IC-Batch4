...
def test_process_todo_created(mock_db):
    ...
    message = {
        "Body": json.dumps(
            {"todo_id": 123, "action": "todo_created", "todo_data": todo_data}
        )
    }
...
def test_process_todo_updated(mock_db):
    ...
    message = {
        "Body": json.dumps(
            {"todo_id": 123, "action": "todo_updated", "todo_data": todo_data}
        )
    }
    process_notification(message)
    mock_db.commit.assert_called_once()
...
def test_process_todo_deleted(mock_db):
    message = {"Body": json.dumps({"todo_id": 123, "action": "todo_deleted"})}
    process_notification(message)
    mock_db.commit.assert_called_once()
...
def test_process_missing_action():
    message = {"Body": json.dumps({"todo_id": 123})}
    with pytest.raises(Exception):
        process_notification(message)


def test_process_invalid_action():
    message = {"Body": json.dumps({"todo_id": 123, "action": "invalid_action"})}
    with pytest.raises(Exception):
        process_notification(message)
...
def test_process_missing_todo_id():
    message = {
        "Body": json.dumps(
            {"action": "todo_created", "todo_data": {"title": "Test Todo"}}
        )
    }
    with pytest.raises(Exception):
        process_notification(message)
